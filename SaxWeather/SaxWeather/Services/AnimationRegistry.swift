//
//  AnimationRegistry.swift
//  SaxWeather
//
//  Phase 6 — Iconography & animation engine.
//
//  `AnimationRegistry` is the single source of truth for resolving
//  which animation (or SF Symbol) should be shown for a given
//  weather condition. It wraps the existing
//  `WeatherAnimationHelper` mapping and layers the customisation
//  knobs from `IconographySpec` on top:
//
//    • `lottieOverrideMap` — per-condition JSON filename overrides
//      (e.g. swap "clear-day" for a custom "my-clear-day.json").
//    • `lottieAnimationSet` — `.bundled` (default Lottie),
//      `.bundledStatic` (force SF Symbols), or `.custom`
//      (only show Lottie when an override exists).
//    • `lottiePlaybackSpeed` — multiplier applied to the
//      `LottieAnimationView.animationSpeed`.
//    • `lottieLoopMode` — `.loop` or `.playOnce`.
//    • `disableWeatherAnimations` + `reduceMotion` — force the
//      SF Symbol fallback path.
//    • `weatherIconStyle` — `.multicolor` / `.monochrome` / `.outline`
//      applied to the SF Symbol rendering mode.
//    • `symbolSet` — `.automatic` / `.filled` / `.outline` SF Symbol
//      variant preference.
//
//  Views should read from this registry (via `ConditionIcon`) rather
//  than calling `WeatherAnimationHelper` directly, so every knob in
//  `IconographySpec` is honoured automatically.
//
...
final class AnimationRegistry {
    ...
    /// inject a test-only registry directly.
    ///
    /// Note: no default parameter here — referencing
    /// `CustomisationRegistry.shared` as a default value would be
    /// evaluated in a nonisolated context, which Swift 6 rejects.
    ...
    /// Resolve the Lottie animation name for a textual condition
    /// (e.g. "Partly Cloudy"). Honours `lottieOverrideMap` first,
    /// then falls back to the bundled mapping in
    ...
    /// Apply `lottieOverrideMap` to a base animation name. The map
    /// is keyed by either the base animation name (e.g.
    /// `"clear-day"`) or the original condition string (e.g.
    /// `"Clear"`), so users can override either way.
