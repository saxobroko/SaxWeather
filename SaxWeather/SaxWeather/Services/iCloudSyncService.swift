...
//  SaxWeather
//
//  iCloud sync for SaxWeather settings.
//
//  Uses `NSUbiquitousKeyValueStore` (Apple's "ubiquitous key-value
//  store") to mirror the active `CustomisationProfile` across every
//  device signed in to the same iCloud account. The store is
//  designed for small, frequently-read preferences — exactly the
//  shape of a SaxWeather theme profile — and is automatically
//  pushed/pulled by the system without any custom networking.
//
//  Sync model:
//
//    • **Push** — every time `CustomisationRegistry` mutates the
//      active profile, the registry calls
//      `iCloudSyncService.shared.push(profile:)`. The service
//      encodes the profile to JSON and writes it to the key-value
//      store under `Self.profileKey`. iCloud then propagates the
//      change to other devices.
//
//    • **Pull** — on launch and whenever iCloud notifies us of an
//      external change (`NSUbiquitousKeyValueStoreDidChangeExternallyNotification`),
//      the service reads the remote profile, compares its
//      `updatedAt` to the local one, and applies the newer one via
//      `CustomisationRegistry.apply(_:)`. Last-modified-wins.
//
//    • **Toggle** — the user can disable sync from the Backup &
//      Restore screen. When disabled, the service stops pushing
//      and ignores external-change notifications. The remote copy
//      is left in place so re-enabling picks up where it left off.
//
//  The service is `@MainActor` because it talks to the
//  `CustomisationRegistry` (which is also `@MainActor`) and to
//  SwiftUI views that observe its `status`.
//
//
